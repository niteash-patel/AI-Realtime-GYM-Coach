...
def inject_local_font(font_path, font_name):
    ...
    extension=os.path.splitext(font_path)[1].lstrip(".")
    format={"otf" : "opentype"}.get(extension , extension)
    mime={"otf": "font/otf"}.get(extension, f"font/{extension}")

    st.markdown(f"""
        <style>
        @font-face {{
            font-family: '{font_name}';
            src: url ('data:{mime};base64, {encoded}') format('{format}');
            font-weight: 100 900;
            font-style: normal;
        }}
        </style>
